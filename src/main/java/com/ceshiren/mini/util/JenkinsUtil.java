...
public class JenkinsUtil {
    ...
    public static OperateJenkinsJobDTO buildWithXML(OperateJenkinsJobDTO operateJenkinsJobDto) throws Exception {
        ...
        System.out.println("params.get(\"testCommand\"):" + params.get("testCommand"));
        jobConfigXml = StringUtils.replace(jobConfigXml,"${MyCommand}",params.get("testCommand"));
        System.out.println("---jobConfigXml----");
        System.out.println(jobConfigXml);
        //3.创建 JenkinsServer
        //创建Jenkins客户端
        JenkinsHttpClient jenkinsHttpClient = new JenkinsHttpClient(new URI(jenkinsUrl),
